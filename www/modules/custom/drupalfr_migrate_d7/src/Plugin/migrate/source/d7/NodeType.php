...
namespace Drupal\drupalfr_migrate_d7\Plugin\migrate\source\d7;
...
class NodeType extends DrupalSqlBase {
  private const CONF_TYPE_NAME = 'types';
  ...
  public function query() {
    ...
    if (!empty($this->configuration[self::CONF_TYPE_NAME])) {
      $query->condition('t.type', $this->configuration[self::CONF_TYPE_NAME], 'in');
    }
  ...
  public function prepareRow(Row $row) {
    ...
    $source_options = $this->variableGet(
      'node_options_' . $type, ['promote', 'sticky']
    );
    $options = [];
