...
do
  echo "Testing existence of ${COLOR_LIGHT_GREEN}\"${FILE}\"${COLOR_NC}"
...
if [ ! "0" -eq ${#FILES_NOT_FOUND[@]} ]; then
  echo "${COLOR_LIGHT_RED}${#FILES_NOT_FOUND[@]} file(s) not found${COLOR_NC}"
...
echo "${COLOR_LIGHT_GREEN}\o/ All files exist${COLOR_NC}"
